...
|---|---|---|---|
| **Binary format** (strict-versioned via `jsono::VERSION`; readers reject other versions loudly; `jsono_version()` reports the current value) | Strict-versioned, fail-loud on mismatch. Repo stance: "backward compat not a constraint." | Third parties will persist `jsono` columns in their own DuckDB/Parquet/DuckLake files. A format bump makes a newer extension reject their stored data loudly. | The current "may change freely" stance is **incompatible with published storage** unless the docs state a policy. Must choose: storage is version-locked (re-write/migrate on upgrade), or old versions stay readable. See question (b). |
...
| **`jsono_version()` / format-version introspection** | Public function returning the current `jsono::VERSION`. | Consumers may branch on it. | Low cost, but it becomes the sanctioned way to gate on format compatibility — tie it to the release scheme (Step 3). |
