...
> [!IMPORTANT]
> In a boolean condition, put an operator extraction in parentheses: write `WHERE cond AND (e ->> '$.k') = 'v'`. The `->` operator binds more loosely than `AND` and `OR`, the same as in core `json` — the arrow token is shared with the [lambda syntax](https://duckdb.org/docs/stable/sql/functions/lambda.html), and a lambda body must bind loosely. Without parentheses, DuckDB parses `cond AND e -> '$.k' = 'v'` as `(cond AND e) -> …` and fails on a cast of the JSONO struct to `BOOLEAN`. DuckDB v2.0 gives `->>` the tighter precedence, so parentheses are what keeps a condition correct for either operator.
...
**Foldable intermediate JSON over a constant literal.** A fully constant expression can fold to `NULL` before the extension optimizer runs. This occurs when the expression extracts an intermediate JSON value from a shredded `jsono(...)` literal and passes it to another core JSON operation. Verified cases are chained `->` / `->>` operations, `json_type(value -> path)`, `json_keys(value -> path)`, and `json_array_length(value -> path)`. Bound values, including table columns and `VALUES` entries, are rewritten correctly. For a constant literal, use one full-path jsono-native call: `jsono_type(value, '$.a')`, `jsono_keys(value, '$.a')`, or `jsono_array_length(value, '$.a')`. For extraction, use one full-path operation such as `value ->> '$.a.b'`.
