...
| `VARCHAR` | string |
| `JSON` | parsed subtree |
| `DATE`, `TIME`, `TIMETZ`, `TIMESTAMP`(`_S`/`_MS`/`_NS`/`TZ`), `UUID`, `ENUM`, `INTERVAL`, `BLOB`, `BIT` | string, rendered exactly as core `to_json` renders it |
| `STRUCT`, `MAP` | object |
| `LIST`, `ARRAY` | array |
| JSONO | composed as a subtree, no text round-trip |
